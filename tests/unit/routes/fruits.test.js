...
describe("POST /api/fruits ", () => {
  it("should return an error if the request body does not have a name", async () => {
    ...
    const response = await request(app).post("/api/fruits").send(invalidBody);
    expect(response.statusCode).toBe(400);
  });

  it("should return fruit object with the provided body", async () => {
    jest.spyOn(Fruit.prototype, "save").mockReturnValue(strawberry);

    const response = await request(app).post("/api/fruits").send(strawberry);
  ...
  it("should return an exception error", async () => {
    ...
    jest.spyOn(Fruit.prototype, "save").mockImplementation(() => {
      throw new Error();
    });

    const response = await request(app).post("/api/fruits").send(testObject);
...
describe("PUT /api/fruits/updatefruit", () => {
  it("should return an error if request body is not valid", async () => {
    const response = await request(app).put("/api/fruits/updatefruit");
  ...
  it("should return an exception if an error occurred while updating", async () => {
    jest.spyOn(Fruit, "findOne").mockImplementation(() => strawberry);
  ...
  it("should return the fruit", async () => {
    ...
    jest.spyOn(Fruit, "updateOne").mockImplementation(() => strawberry);
